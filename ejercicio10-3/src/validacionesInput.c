...
//validacion utilizada para la confrimacion de salida del programa
int ValidacionSalir(char* pResultado, char* mensaje, char* mensajeError, char valor1, char valor2, int reintentos){
	int retorno;
	char bufferChar;
	retorno = -1;
	if(pResultado != NULL && mensaje != NULL && mensajeError != NULL && reintentos >= 0){
		do{
			...
			fflush(stdin);//windows
			scanf("%c", &bufferChar);
			if(bufferChar == valor1 || bufferChar == valor2){
				*pResultado = bufferChar;
				retorno = 0;
				break;
			} else{
